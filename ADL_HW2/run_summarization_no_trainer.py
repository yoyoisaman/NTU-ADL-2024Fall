...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        result = {}
        score = metric_2(predictions, references)
        rouge1.append((100 * score['rouge-1']['f']))
        rouge2.append((100 * score['rouge-2']['f']))
        rougeL.append((100 * score['rouge-l']['f']))
        train_loss.append(total_loss.item() / len(train_dataloader))
        # print()
        # print("=============")
        # print(result)
        # print("=============")
        print(f"epoch: {epoch}, loss: {train_loss[-1]}, rouge1: {100 * score['rouge-1']['f']}, rouge2: {100 * score['rouge-2']['f']}, rougel: {100 * score['rouge-l']['f']}")
        ...
        if args.output_dir is not None and (epoch+1) % 2 == 1:
            epoch_output_dir = os.path.join("/root/autodl-tmp", f"model_epoch_{epoch+1}")
            accelerator.wait_for_everyone()
            unwrapped_model = accelerator.unwrap_model(model)
            unwrapped_model.save_pretrained(
                epoch_output_dir, is_main_process=accelerator.is_main_process, save_function=accelerator.save, safe_serialization=False
            )
            if accelerator.is_main_process:
                os.makedirs(epoch_output_dir, exist_ok=True)
                tokenizer.save_pretrained(epoch_output_dir)
                print(f"模型和 tokenizer 已保存到 {epoch_output_dir}")
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            score = {'Train loss': train_loss, 'rouge1': rouge1, 'rouge2': rouge2, 'rougeL': rougeL}
