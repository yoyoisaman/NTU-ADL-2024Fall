...
def main():
    ...
    # 選項 ending1 ending2 ending3 ending4
    ...
    def preprocess_function(examples):
        ...
        # idx-> context_file 
        for idx in examples[ending_names]:
            cand = []
            for i in idx:
        ...
        for i in range(len(examples['id'])):
            paragraphs = examples['paragraphs'][i]
            relevant = examples['relevant'][i]
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            all_results["train_loss"] = total_loss.item() / len(train_dataloader)
            with open(os.path.join(args.output_dir, "all_results.json"), "w") as f:
                json.dump(all_results, f)
            with open(os.path.join(args.output_dir, "epoch_results.json"), "w") as f:
