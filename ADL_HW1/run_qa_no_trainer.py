...
def main():
    ...
    # print("====================")
    # ['relevant', 'paragraphs', 'question', 'answer', 'id']
    # print(column_names)    
    # print("====================")    
    ...
    def prepare_train_features(examples):
        ...
        # '在19世紀雙星觀測所獲得的成就使重要性也增加了。在1834年，白塞爾觀測到天狼星自行的變化，因而推測有一顆隱藏的伴星；愛德華·皮克林在1899年觀測開陽週期性分裂的光譜線時發現第一顆光譜雙星，週期是104天。天文學家斯特魯維和舍本·衛斯里·伯納姆仔細的觀 察和收集了許多聯星的資料，使得可以從被確定的軌道要素推算出恆星的質量。第一個獲得解答的是1827年由菲利克斯·薩瓦里透過望遠 鏡的觀測得到的聯星軌道。對恆星的科學研究在20世紀獲得快速的進展，相片成為天文學上很有價值的工具。卡爾·史瓦西發現經由比較 視星等和攝影星等的差別，可以得到恆星的顏色和它的溫度。1921年，光電光度計的發展可以在不同的波長間隔上非常精密的測量星等。阿爾伯特·邁克生在虎克望遠鏡第一次使用干涉儀測量出恆星的直徑。'
        # print(relevant)
        # print("====================")  

        # {'start': 335, 'text': '賽德娜的表面太過寒冷'}
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            answers = ans[sample_index]
            # Cant find the answer in the range 
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        ans = [{'answer_start': example[answer_column_name]['start'], 'text':example[answer_column_name]['text']} for example in examples]
    ...
    em_score = []
    epoch_tain_loss_score = []
    epoch_val_loss_score = []
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                epoch_tain_loss_score.append(total_loss.item() / len(train_dataloader))
        ...
        logger.info(f"Evaluation metrics: {eval_metric} train_loss: {total_loss.item() / len(train_dataloader)} eval_loss: {val_loss.item() / len(eval_dataloader)}")
        
        em_score.append(eval_metric['exact_match'])
        epoch_val_loss_score.append(val_loss.item() / len(eval_dataloader))
        
    
    score = {'Val EM Score': em_score, 'Train Loss': epoch_tain_loss_score, 'Val Loss':epoch_val_loss_score}
