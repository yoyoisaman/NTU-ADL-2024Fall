...
from utils import get_bnb_config, get_fewshot_prompt
import argparse
import random


def perplexity(
    model, tokenizer, data, max_length=2048, num_shots=5,
):
    data_size = len(data)
    ...
    for x in data:
        # 從其他資料中選擇範例(排除當前的資料點)
        other_data = [d for d in data if d != x]
        few_shot_examples = random.sample(other_data, num_shots)
        instruction = get_fewshot_prompt(x["instruction"], few_shot_examples)
...
if __name__ == "__main__":
    ...
    model.eval()
    ppl = perplexity(model, tokenizer, data, num_shots=args.num_shots)
